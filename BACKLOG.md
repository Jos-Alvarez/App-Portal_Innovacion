...
| La librería de cache revalidable del ADR 0007 será **SWR**, no React Query. Pesa ~4 KB, no necesita provider, y trae de fábrica la revalidación al foco y por intervalo que el ADR pide textualmente. | Cierra la disyuntiva que el ADR 0007 dejaba abierta ("SWR o React Query"). Sin cambio de ADR. |
| La **adopción de SWR se movió del ítem #4 al #8**. Al implementar el #4 no existía todavía ningún fetch de cliente en el portal: la pantalla `/` es un Server Component y el primer consumidor real de datos en cliente es el dashboard del #8. Configurar la capa de cache sin nada que cachear habría significado diseñar su forma a ciegas. | **Backlog actualizado** (alcance del #4 y del #8). No incumple el ADR 0007: mientras no haya cache en cliente, no hay nada que pueda quedar obsoleto. |
| El **ítem #6 incluye la baja** de procesadores, que no figuraba en su alcance. PRD, TECH-DESIGN y este backlog la omitían los tres para procesadores y la incluían los tres para enlaces, así que la omisión parecía deliberada. Se incluye igual: el guard de autorización ya deniega recursos con `activo = false` y trata `enlace` y `procesador` de forma genérica, así que la baja no cuesta código nuevo allí; sin ella, sacar un procesador de circulación exigiría revocar sus asignaciones una por una en el #7. | **Backlog actualizado** (alcance del #6). Baja lógica, idéntica a la del #5. |
| El párrafo de **protección CSRF del ADR 0007 era inexacto**: atribuía la protección de las rutas de mutación del portal a los tokens CSRF de Auth.js, que solo cubren `/api/auth/*`. La protección real es `SameSite=Lax` en la cookie de sesión. | **ADR 0007 actualizado** con una nota de corrección. La decisión no cambia; sí el mecanismo que la sustenta. |
...
| 4 | Autorización en servidor | portal | Guard por request contra BD (`es_admin` y filas de asignación) y pantalla 403 de DESIGN.md. **La revalidación de la cache del cliente se movió al #8** (ver notas de secuencia) | #3 | — |
| 5 | Catálogo de enlaces (admin) | portal | Alta, edición y baja de `enlace` con validación de formato de URL y tipo app \| agente | #4 | — |
| 6 | Catálogo de procesadores (admin) | portal | Alta, edición **y baja** de la fila `procesador`: nombre, `clave_procesador`, formatos aceptados, `tamano_max`, y el contrato de E/S (`entradas_min`, `entradas_max`, `tamano_max_total`, `salida_esperada`) | #4 | — |
...
| 8 | Dashboard del colaborador | portal | Vista unificada de lo asignado con chips de tipo, apertura de enlace externo en pestaña nueva con registro de `evento_uso` tipo `apertura`, estado vacío, y **la adopción de SWR con la política de revalidación del ADR 0007** (al foco y por intervalo), movida desde el #4 | #7 | — |
